...
class NavSimuleringSpecMapperV2(val personService: GeneralPersonService) {
    ...
    fun fromSimuleringSpecV2(
        ...
        return SimuleringSpec(
            ...
            pre2025OffentligAfp = tidsbegrensetOffentligAfpSpec(source),
            erAnonym = false,
            ignoreAvslag = false,
    ...
    private companion object {
        ...
        private fun tidsbegrensetOffentligAfpSpec(simuleringSpec: NavSimuleringSpecV2): Pre2025OffentligAfpSpec? =
            if (simuleringSpec.simuleringType == NavSimuleringTypeSpecV2.AFP_ETTERF_ALDER)
                Pre2025OffentligAfpSpec(
                    afpOrdning = simuleringSpec.afpOrdning ?: missingAfpOrdning(),
        ...
        private fun avdoed(source: NavSimuleringSpecV2): Avdoed? =
            source.fnrAvdod?.let {
                Avdoed(
                    ...
                    doedDato = source.dodsdato?.toNorwegianLocalDate() ?: missingDoedsdato(),
                    erMedlemAvFolketrygden = source.avdodMedlemAvFolketrygden == true,
                    harInntektOver1G = source.inntektAvdodOver1G == true
                )
            }

        private fun missingAfpOrdning(): Nothing {
            throw BadSpecException("afpOrdning is required", ProblemType.ANNEN_KLIENTFEIL)
        }

        private fun missingDoedsdato(): Nothing {
            throw BadSpecException("dodsdato is required", ProblemType.ANNEN_KLIENTFEIL)
        }
    }
}