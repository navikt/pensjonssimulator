...
public class OmsorgspoengLogUtil {
    private static final String finnesIkke = " - ";
    private static final String nl = String.format("%1$3s", System.getProperty("line.separator") + " ");
    ...
    private static String poengtallListeString(List<Omsorgsopptjening> list) {
        ...
        buf.append(String.format("%1$3s %2$6s %3$5s %4$5s %5$7s %6$7s %7$6s %8$6s %9$5s %10$1s %11$s", nl, "type", "ar", "pp", "pi", "pia", "gv", "brukt", "maksUfg", "",
                "Merknader"));
        for (Omsorgsopptjening pt : list) {
            if (pt instanceof Poengtall) {
                buf.append(nl).append(poengtallString(pt));
            }
            if (pt instanceof OpptjeningUT) {
                buf.append(nl + opptjeningUTString(pt));
            }
        }
        return buf.toString();
    }
    ...
    private static String opptjeningUTString(Omsorgsopptjening o) {
        if (!(o instanceof OpptjeningUT)) {
            return finnesIkke;
        }
        OpptjeningUT p = (OpptjeningUT) o;
        StringBuilder buf = new StringBuilder();
        String ial = (p.getInntektIAvtaleland() ? "IAL" : finnesIkke);
        String omsorg = (p.getOmsorg() ? "OMSORG" : finnesIkke);
    ...
    private static String refarString(Ref_pp r) {
        if (r.pt != null) {
            return String.format("%1$12d %2$2s", r.pt.getOpptjeningsar(), "  ");
        }
    ...
    private static String refppString(Ref_pp r) {
        if (r.pt != null) {
            return String.format("%1$12.2f %2$2s", r.pt.getVerdi(), ((r.pt.getInntektIAvtaleland()) ? "* " : "  "));
        }
    ...
    public static String refppListString(List<Ref_pp> o) {
        ...
        if (o != null) {
            for (Ref_pp p : o) {
                if (p.pt != null) {
                    s = s + String.format("%1$12.2f %2$2s", p.pt.getVerdi(), ((p.pt.getInntektIAvtaleland()) ? "* " : "  "));
                }
            }
        }
        return s.trim();
