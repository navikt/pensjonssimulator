...
public class OmsorgspoengTrygdeavtaleRek {
     ...
     *
     * @param opptjeningUTs opptjening fra Uføretrygd
    ...
    public OmsorgspoengTrygdeavtaleKombinasjon finnBeste3Av5_trygdeavtale(List<OpptjeningUT> opptjeningUTs) {
        /*
         * Passer på at antall plasser kan være mindre enn 5 uten å gi nullpointer.
         * Setter antall aktuelle år til maksimalt 5, men mindre dersom vi ikke har 5 år i listen.
         */
        int antallPlasserMax5 = Math.min(5, opptjeningUTs.size());
        ...
        Parametre param = new Parametre(opptjeningUTs, finnSenesteArFraListe(opptjeningUTs), antallPlasserMax5);
        ...
        klargjorParametreFullIal(param, antallPlasserMax5);
        //Utleder alle kombinasjoner som kan være beste kombinasjon.
        param = finnBesteKombinasjoner(param, new OmsorgspoengTrygdeavtaleKombinasjon(antallPlasserMax5, false), 0);
        ...
        param = finnBesteKombinasjonerMedUtvidetOmfang(param, new OmsorgspoengTrygdeavtaleKombinasjon(antallPlasserMax5, false), param.senesteAr);
        ...
        finnRelevanteAr(besteKombinasjon, opptjeningUTs);
    ...
    private void finnInntektsar(Parametre param, int vindu) {
        param.inntektsAr = new HashMap<>();
        param.alleArListe.sort(new OmsorgspoengCompareUtil.SynkendeArstallComparator());
        int i = 0;
        int antallInntektAr = 1;
        while (antallInntektAr <= vindu && i < param.alleArListe.size()) {
            if (!param.alleArListe.get(i).getOmsorg()) {
     ...
     */
    private void finnRelevanteAr(OmsorgspoengTrygdeavtaleKombinasjon kombinasjon, List<OpptjeningUT> opptjeningUTs) {
        List<OpptjeningUT> relevanteAr = new ArrayList<>();
        if (kombinasjon.getUtvidetOmfang() == null) {
            ...
            for (OpptjeningUT o : opptjeningUTs) {
                if (o.getOpptjeningsar() > tidligsteAr) {
                    ...
                    if (o.getOmsorg() && !finnesArIListe(kombinasjon.getUtvidetOmfang().getKombinasjon(), o.getOpptjeningsar())) {
                        relevanteAr.add(o);
                    }
                }
            }
    ...
    private void finnOmsorgsarPermutasjoner(Parametre param, int vindu) {
        ...
        List<OpptjeningUT> kunNonZeroIalAr = new ArrayList<>();
        List<OpptjeningUT> kunNonZeroIkkeIalAr = new ArrayList<>();
        List<OpptjeningUT> kunZeroAr = new ArrayList<>();
        ...
        for (OpptjeningUT o : param.alleArListe) {
            if (!o.getInntektIAvtaleland() && o.getVerdi() > 0.0) {
                kunNonZeroIkkeIalAr.add(o);
            } else if (o.getInntektIAvtaleland() && o.getVerdi() > 0.0) {
        ...
        for (int i = 0; i <= vindu; i++) {
            ...
            nStorsteOmsorgsarMellom = finnNBesteOmsorgsarMellom(kunNonZeroIkkeIalAr, startAr, stoppAr, antallMuligeAr, new OmsorgspoengCompareUtil.PoengtallComparator());
            nStorsteIalArMellom = finnNBesteOmsorgsarMellom(kunNonZeroIalAr, startAr, stoppAr, antallMuligeAr, new OmsorgspoengCompareUtil.PoengtallComparator());
            nZeroArMellom = finnNBesteOmsorgsarMellom(kunZeroAr, startAr, stoppAr, antallMuligeAr, new OmsorgspoengCompareUtil.IalPoengtallComparatorOmsorgsopptjeningTrygdeavtale());
            List<List<OpptjeningUT>> permutasjoner = new ArrayList<>();
            ...
            permutasjoner.sort(new OmsorgspoengCompareUtil.StorrelseOgLeksikografiskArstallComparator());
            //Lagre på parametere
    ...
    private List<List<OpptjeningUT>> genererOmsorgsarAlternativer(
            ...
            int antallPlasser) {
        List<List<OpptjeningUT>> returListe = new ArrayList<>();
    ...
    private List<OpptjeningUT> finnNBesteOmsorgsarMellom(List<OpptjeningUT> opptjeningUTs, int ar1, int ar2, int antall, Comparator comp) {
        List<OpptjeningUT> omsorgsArIIntervall = new ArrayList<>();
        //Legger alle omsorgsårene mellom forste og siste år inn i en liste
        for (OpptjeningUT o : opptjeningUTs) {
            if (o.getOpptjeningsar() < ar1 && o.getOpptjeningsar() > ar2 && o.getOmsorg()) {
                omsorgsArIIntervall.add(o);
            }
        }
        List<OpptjeningUT> deNBesteOmsorgsarene = new ArrayList<>();
        omsorgsArIIntervall.sort(comp);
    ...
    private Integer finnTidligsteArFraListe(List<OpptjeningUT> liste) {
        ...
        for (OpptjeningUT o : liste) {
            if (tidligsteAr == null || o.getOpptjeningsar() < tidligsteAr) {
    ...
    private Integer finnSenesteArFraListe(List<OpptjeningUT> liste) {
        ...
        for (OpptjeningUT o : liste) {
            if (senesteAr == null || o.getOpptjeningsar() > senesteAr) {
    ...
    private class Parametre {
        ...
        public Parametre(List<OpptjeningUT> alleArListe, int senesteAr, int vindu) {
            ...
            besteOmsorgsarUtvidelser = new HashMap<>();
            inntektsAr = new HashMap<>();
            kombinasjonsliste = new ArrayList<>();
